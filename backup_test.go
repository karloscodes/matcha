...
func TestCleanupOldBackups(t *testing.T) {
	t.Run("keeps only last 3 backups", func(t *testing.T) {
		tmpDir := t.TempDir()
		backupDir := filepath.Join(tmpDir, "testapp", "backups")
		os.MkdirAll(backupDir, 0755)

		m := New(Config{Name: "testapp", AppImage: "test:latest", DataDirBase: tmpDir})
...
func TestCreateBackup(t *testing.T) {
	t.Run("fails when database does not exist", func(t *testing.T) {
		tmpDir := t.TempDir()
		os.MkdirAll(filepath.Join(tmpDir, "testapp"), 0755)
